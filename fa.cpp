...
static void
decr_excise(kct_t const *const kc, uint32_t i)
{
    dbg |= i == dbgndxkct ?  8 : 0;
 ...
 */
        ...
        t = ct == 1ul ? 0 : kc->kct[i]++; // passed this key, if not yet unique and b2pos
        ...
        kc->kct[i] ^= -(ct == 1ul) & (kc->kct[i] ^ r.infior) & STRAND_POS;
