        ...
        *k -= ONE_PENDING - 1ul;
        ...
        if (IS_FIRST(k)) {
            if (IS_DISTINCT(k)) {
                ...
                ASSERT(k[1] & MARKED, goto out, "MARKED not obsolete");
            }
        } else {
            ASSERT(t, goto out, "nt offset -1 (below)?");
...
static int
decr_excise_all(kct_t C*C kc, uint64_t *C k, uint64_t C*C kct, unsigned const end)
{
    int err = 0;
...
out:
    EPQ(err, "before uniq %lu", TSO_NT(kct));
    return err;
}
