...
cerr << key << endl << flush;
        ...
        if (k == kh_end(H)) { /* new key */
            for (unsigned i = 0; i != 4; ++i, ++b) *b = '\0'; /* keycount */
            p = b;
            for (unsigned i = 0; i != 4; ++i, ++b, l >>= 8)
                *b = l & 0xff; /* key offset, will be tagged along to last value */

            while ((*b++ = *key) != '\0') ++key;
            *b++ = '\0';
cerr << "key at: " << p - s - 4 << endl << flush;
            k = kh_put(UQCT, H, p - s - 4, &is_err);
            is_err = (is_err < 0);
            if_ever (is_err) return;
cerr << "kh_size: " << kh_size(H) << endl << flush;
            l = b - s;

        } else {
            size_t i = kh_key(H, k);
cerr << "already observed key at " << i << ":\n";
            p = s + i;
cerr << p + 8 << "\n";
        ...
        *b = '\0'; *++b = '\0'; *++b = '\0'; *++b = '\0'; ++b;
        /* transfer former greatest offset to before last value, put last offset before key */
        for (unsigned i = 0; i != 4; ++i, ++b, ++p) {
            *b = *p;
            *p = l & 0xff;
cerr << i << ":" << (l & 0xff) << endl;
            l >>= 8;
